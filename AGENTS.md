...
- Before modifying any file in `The-Interdependency/The-Interdependency.github.io`, load `website-builder-journal/SKILL.md`. Every website change transaction must append a new `By the builder` record with date, time, exact runtime model, and model-discretionary prose before terminal validation; read-only inspection is the non-trigger.
