...
| `adaptive-elicitation/` | procedural | — | Context-first semi-structured interviewing. Harvest known facts before asking, ask one neutral construct at a time, adapt from each answer, consume volunteered facts, and preserve unresolved material as `hmmm`. No metadata block. |
...
| `website-builder-journal/` | procedural | — | Requires every `interdependentway.org` website modification transaction to append a By the builder entry with date, time, exact runtime model, and model-discretionary minimal prose; prior entries remain immutable and the public journal remains collapsible. |
