...
- **Procedural skills** define an agent behaviour with no msdmd block. They state the doctrine
  they enforce and the output shape they produce. `canon/`, `visitor-intro/`, `char-compress/`, `agent-instantiation/`, `a0p-instancing/`, `plain-lens/`, `gonal-morphology/`, `meta/`, `the-interdependency/`, `interdependent-work-graph/`, `loop-eng/`, and `skill-build/` are the examples.
