...
| Root axiom | Permitted Relation | Conductive path enables `I=\Delta V/R` | Membrane permeability enables `J=P(C_{out}-C_{in})` | Graph adjacency enables `d\mathbf x/dt=-L\mathbf x` | Graph adjacency | Change requires available relation, path, coupling, or medium. | Do not infer movement from difference alone. |
